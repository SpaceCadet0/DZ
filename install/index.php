...
define('REQUIRE_MIN_VERSION_PHP', '5.6');
...
if (isset($_REQUEST['license'])) {
    ...
    $eula = file_get_contents('https://www.flynax.ru/installation-eula.html', false, $context);
...
/* load phrase's */
// TODO: Replace the hardcode to an dynamic variable
$phrase = json_decode(file_get_contents(RL_INSTALL . 'lang/en.json'), true);
/* load phrase's end */
...
$requires = array(
    ...
    'magic_quots'      => array(
        ...
        'value'       => get_magic_quotes_gpc() ? $phrase['enabled'] : $phrase['disabled'],
        ...
        if ($_SESSION['requires_access_info'] && $_SESSION['database_info'] && $_SESSION['advanced_info'] && $_SESSION['admin_info']) {
            if (!$_SESSION['main_dump_loaded']) {
                ...
                if ($rlDb->lastErrno()) {
                    ...
                    $queryql_dump = fopen(RL_INSTALL . 'mysql/dump.sql', 'r');
                                ...
                                if (false !== strpos($query_sql, '{timezone}')) {
                                    ...
                                    if ($phpTimezone && $l_timezone) {
                                        foreach($l_timezone as $shortName => $zone) {
                                            ...
                                            if ($locationsInZone 
                                                && false !== strpos($locationsInZone, end(explode('/', $phpTimezone)))
                                            ) {
                                                $timezone = $shortName;
                                                break;
                                            }
...
<div id="main_container">
    <div id="header">
        ...
        <div id="vav_bar">
            <a target="_blank" href="https://www.flynax.ru"><?php _phrase('link_name_flynax');?></a>
            <a target="_blank" href="http://forum.flynax.com"><?php _phrase('link_name_forum');?></a>
            <a target="_blank" href="http://forum.flynax.com/showthread.php?47-Software-Installation-Manual"><?php _phrase('link_name_manual');?></a>
        </div>
        <div class="clear"></div>
    </div>
    <div id="body">
