            ...
            function(response) {
                ...
                if (response) {
                    ...
                    if (response.status && response.status == 'OK') {
                        if (response.message) printMessage('notice', response.message);
                        if (typeof callbackSuccess == 'function') callbackSuccess(response);
                    }

                    // the request failed
                    if (response.status && response.status == 'ERROR') {
                        if (response.message) printMessage('error', response.message);
 ...
 * @param {string} action - Action from GET (add/edit)
 */
function field_types(type, action)
{
    ...
    if (action == 'add' || (action == 'edit' && type == 'accept')) {
        var reqiured = $('[name="required"]');
        var reqiured_tr = $(reqiured).closest('tr');
        var reqiured_value = $(reqiured).prop('checked');

        var details_page = $('[name="details_page"]');
        var details_page_label = $(details_page).closest('label');
        var details_page_value = $(details_page).prop('checked');

        var $googleMapTr    = $('[name="map"]').closest('td.field').closest('tr');
        var $addListingPage = $('[name="add_page"]').closest('tr');

        if (type == 'accept') {
            $(reqiured_tr).fadeOut('fast');
            reqiured_value = true;

            $(details_page_label).fadeOut('fast');
            details_page_value = false;

            $googleMapTr.hide();
            $addListingPage.hide();
        } else {
            reqiured_value = false;
            $(reqiured_tr).fadeIn();

            details_page_value = true;
            $(details_page_label).fadeIn();
        }

        $(reqiured).each(function(){
            if (reqiured_value) {
        ...
        if (details_page_value) {
            $(details_page).attr('checked', true);
        } else {
            $(details_page).removeAttr('checked');
        }
    }
}
