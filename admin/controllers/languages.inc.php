...
if ($_GET['q'] == 'ext_list') {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'search') {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS `ID`, `Code`, `Module`, CONCAT({$concat_fields}) AS `Key`, `Value` FROM `{db_prefix}lang_keys` ";
        $sql .= "WHERE {$where} AND `Status` = 'active' AND `Module` <> 'email_tpl' AND `Key` NOT LIKE 'data_formats+name+%' ";
        ...
        $lang_count = $rlDb->getRow("SELECT COUNT(`ID`) AS `count` FROM `{db_prefix}lang_keys` WHERE `Code` = '{$langCode}' AND `Status` = 'active' AND `Module` <> 'email_tpl' AND `Key` NOT LIKE 'data_formats+name+%'");
    }
