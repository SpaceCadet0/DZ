...
if ($_GET['q'] == 'ext') {
    ...
    $sql .= "LEFT JOIN `{db_prefix}lang_keys` AS `T3` ON CONCAT('categories+name+',`T1`.`Key`) = `T3`.`Key` AND `T3`.`Code` = '" . RL_LANG_CODE . "' ";
    ...
    foreach ($data as $key => $value) {
        ...
        if ($data[$key]['ID'] == $rlListingTypes->types[$value['Type']]['Cat_general_cat']) {
            $data[$key]['name'] = $data[$key]['name'] . ' <span>(' . $lang['general_category'] . ')</span>';
        }
    }
