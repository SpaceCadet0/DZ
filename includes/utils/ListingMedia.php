...
class ListingMedia
{
    ...
    private static function prepare(&$item)
    {
        ...
        if ($item['Type'] == 'video') {
            ...
            if ($item['Original'] == 'youtube') {
                ...
                $item['href'] = "//www.youtube.com/embed/{$item['Photo']}?autoplay=1";
    ...
    public static function buildName($listing_id, $postfix, $position = 0, $extension = '', $upload_dir = '')
    {
        ...
        if (!$options['seo_picture_names']) {
            ...
            $other_part_len = strlen($index) + 2 + ($extension ? strlen($extension) + 1 : 5);
            $other_part_len += strlen($upload_dir);

            $title_maxlen = $options['seo_picture_names_maxlength'] - $other_part_len;
            $listing_title = preg_replace('/\s+?(\S+)?$/', '', substr($listing_title, 0, $title_maxlen));
