...
class rlCommon extends reefless
{
    ...
    /**
     * @deprecated 4.7.0 - See pageMetaTags() method
     */
    public static function pageTitle(&$page_info, $bread_crumbs) {}
                ...
                if (!empty($field['Condition'])) {
                    ...
                    if ($field['Key'] == 'Category_ID') {
                        ...
                        if ($this->listings[$id] && $this->listings[$id]['name']) {
                            ...
                            $last = $lang['categories+name+' . $cat_info['Key']];
                        }
    ...
    public function getBlocks()
    {
        ...
        if (in_array($page_info['Controller'], array('search_map'))) {
            return;
        }
        ...
        if ($config['mod_rewrite']) {
            ...
            if (!empty($category_path)) {
                ...
                $GLOBALS['rlValid']->sql($category_path);

                /* get category id */
                $category_id = (int) $this->getOne('ID', "`Path` = CONVERT('{$category_path}' USING utf8) AND `Status` = 'active'", 'categories');
            }
        } else {
        ...
        if ($page_info['Controller'] == 'listing_type') {
            ...
            if ($page_info['ID'] == 25) {
                $add_where_no_cat = "OR `Category_ID` = '' ";
            }

            $sql = "SELECT `ID`, `Key`, `Side`, `Type`, `Content`, `Tpl`, `Header`, `Position`, `Plugin` FROM `{db_prefix}blocks`";
            $sql .= "WHERE (`Sticky` = '1' OR ";
            $sql .= "(FIND_IN_SET( '{$page_info['ID']}', `Page_ID` ) > 0 ";

            if ($category_id) {
                $sql .= "AND ((`Category_ID` <> '' AND FIND_IN_SET('{$category_id}', `Category_ID`) > 0) OR `Cat_sticky` = '1' {$add_where}{$add_where_no_cat}) ";
            } else {
                $sql .= "AND (`Category_ID` = '' ) ";
            }
            ...
            $tmp_blocks = $this->fetch(array('ID', 'Key', 'Side', 'Type', 'Content', 'Tpl', 'Header', 'Position', 'Plugin'), array('Status' => 'active'), "AND (FIND_IN_SET( '{$page_info['ID']}', `Page_ID` ) > '0' OR `Sticky` = '1') {$where} ORDER BY `Position`", null, 'blocks');
        }
        ...
        if (!empty($tmp_blocks)) {
            ...
            $blocks = $GLOBALS['rlLang']->replaceLangKeys($blocks, 'blocks', array('name'));
