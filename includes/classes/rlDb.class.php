...
class rlDb extends rlDatabase
{
     ...
     */
    public function createTable($table, $raw_sql, $prefix = RL_DBPREFIX, $properties = '')
    {
    ...
    public function updateOne($data, $table, $html_fields = null)
    {
        ...
        if (!is_array($data['fields']) || !is_array($data['where'])) {
            trigger_error(__METHOD__ . " failed, data['fields'] and data['where'] are required", E_USER_ERROR);
