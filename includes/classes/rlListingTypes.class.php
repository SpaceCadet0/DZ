...
class rlListingTypes extends reefless
{
        ...
        foreach ($allowed_type_keys as &$allowed_type) {
            if ($this->types[$allowed_type]) {
                ...
                if ($single_id = $this->types[$allowed_type]['Cat_single_ID']) {
                    $allowed_types[$allowed_type]['Single_category'] = \Flynax\Utils\Category::getCategory($single_id);
                }
            }
        }
    ...
    public function updateSingleID($type = null) {
        ...
        $categories = \Flynax\Utils\Category::getCategories($type);
        $update = array(
            'fields' => array(
                'Cat_single_ID' => count($categories) === 1 ? current($categories)['ID'] : 0
            ),
            'where'  => array('Key' => $type)
        );
