...
class rlListings extends reefless
{
    ...
    public function getShortDetails($id, $plan_info = false)
    {
        ...
        $sql = "SELECT `T1`.*, `T3`.`Type` AS `Listing_type`, `T3`.`Path` AS `Category_path`, `T3`.`Parent_IDs` ";
        if ($plan_info) {
        ...
        switch ($mode) {
            case 'category':
                ...
                if ($config['multilingual_paths'] && RL_LANG_CODE !== $config['lang']) {
                    ...
                    if ($multilingualPath && false === strpos($_SERVER['QUERY_STRING'], $multilingualPath)) {
            ...
            case 'listing':
                ...
                $request_base = $GLOBALS['domain_info']['scheme'] . '://' . $_SERVER['HTTP_HOST'] . '/' . $rlDir;
                ...
                $real_base = $parsed['scheme'] . '://' . $parsed['host'] . '/' . $rlDir;
