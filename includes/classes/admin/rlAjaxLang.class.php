...
class rlAjaxLang extends reefless
{
    ...
    public function ajax_addLanguage($data)
    {
        ...
        if (!utf8_is_ascii($iso_code)) {
            ...
            $lang_exist = $rlDb->fetch('*', array('Code' => $iso_code), null, null, 'languages');
        ...
        if (!empty($error)) {
            ...
            if (!empty($error)) {
                ...
                if ($GLOBALS['config']['multilingual_paths']) {
                    ...
                    $rlDb->addColumnToTable("Path_{$iso_code}", $where, 'categories');
                    $rlDb->addColumnToTable("Path_{$iso_code}", $where, 'pages');
                }
