...
class rlAdmin extends reefless
{
            ...
            switch ($value['Key']) {
                ...
                case 'home_gallery_box':
                    if ($tpl_settings['home_page_gallery']) {
                        ...
                        $value['Values'] = $rlLang->replaceLangKeys($boxes, 'blocks', array('name'));
                    } else {
                        unset($configs[$key]);
                    }
                    break;
     ...
     * @package xAjax
     *
     * @param string $ids     - listings ids, ex: 250,174,501,...
    ...
    public function ajaxCheckForUpdate($key = false)
    {
        ...
        if ($update_available) {
            $link = '<a target="_blank" href="https://www.flynax.ru/plugins/' . $log['path'] . '.html#changelog">' . $plugin_info['Name'] . '</a>';
